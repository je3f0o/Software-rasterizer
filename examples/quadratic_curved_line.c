...
static void bezier_curve(Canvas* canvas) {
  ...
  for (int i = 0; i < STEP; ++i) {
    float pt = (float)i/STEP;
    float x1 = lerp(points[0].circle.x, points[1].circle.x, pt);
    float y1 = lerp(points[0].circle.y, points[1].circle.y, pt);
    float x2 = lerp(points[1].circle.x, points[2].circle.x, pt);
    float y2 = lerp(points[1].circle.y, points[2].circle.y, pt);
